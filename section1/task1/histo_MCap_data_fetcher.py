'''
COMMENTS ON TASK 1:
1. while urllib is a built-in python http request library, sometimes it's 
difficult and less flexible to use. Take a look at python's requests library, 
which will save you a lot of time getting rid of issues like ssl...
import urllib.request import urllib.parse import urllib.error to
import requests

2. probably you want to create a function accepting params like start_date and 
end_date, so you can easily download parts of data within your desired period.
def get_data(url_parms, serviceurl, ctx, earliest_timestamp):

3. consider using pd.DataFrame() constructor to load json(dict) directly 
into pandas dataframe and format data there.
def extract_from_json_to_df(js_data, df):

Overall, good work for the research for cryptocompare's data api, now you 
know how to get market data from data source using http request. 
My advice will be trying to modularize your codes to make them more 
user-friendly and easily reusable.
'''
...
if __name__ == "__main__":
    ...
    #df = cc_api.getHistoData('BTC', 'USDT', '1h', start_time="2017-04-01", end_time="2020-04-01", e='binance')
    #df = cc2bt(df)
    # with open(os.path.join(DATA_DIR, "BTC_USDT_1h_20170401_20200401.csv"), 'w') as csv_file:
